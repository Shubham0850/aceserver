...
const createCode = async () => {
  ...
  var isExists = await StockModel.findOne({
    code: ddmmyyyy + get6digits(todaysStockLength++),
  });
  while (isExists) {
    isExists = await StockModel.findOne({
      code: ddmmyyyy + get6digits(todaysStockLength),
    });
...
const createPacks = async ({
  ...
  try {
    for (var i = 0; i < numberOfPacks; i++) {
      if (isLoose) {
        const looseStock = await StockModel.findOne({
          code: 'loose',
          branch,
          godown,
        });
        if (!looseStock) {
          const newLooseStock = new StockModel({
            quantity: 0,
            productId,
            branch,
            godown,
...
const getBy = async query => {
  try {
    const stock = await StockModel.find(query)
      .populate('productId')
      .populate('inwardFrom')
      .populate('inwardFrom.salesman')
      .populate('inwardFrom.party')
...
const getTodaysStock = async () => {
  try {
    ...
    const stocksOfToday = await StockModel.find({
